...
# set reporting date
####
reporting_date <- as_date("2023-08-01")
...
excel_load <- read_excel("NMC_august.xlsx")%>%filter(!Case_ID %in% "230812_4385687")



# run cleanign script
source("NMC_merging.R")
...
data_dup32<- stata2script(tibble_df)
...
write.xlsx(df, file = "auguust_NMC_cleaned.xlsx")
df
```
...
intro_blurb <- paste0(
"A total of ", nrow(df)," cases were diagnosed and notified to the NMCSS in ", reporting_date_month_year,". There were ", nrow(df1)," current notifications and ", nrow(df2), " back captured notifications (**see Appendix no.3 for definitions**). The majority were category 2 conditions n(%) ", cat2,". The provinces with the highest number of notifications were", most_common_prov, ", ", sec_common_prov, ", and ", thir_common_prov,". The provinces with the least number of notifications were ", least_common_prov,", and ", sec_least_common_prov,". (**Figure 1**) There were ", sum(df2$'reporting' == 1, na.rm = T), " back captured clinical notifications diagnosed between `", min_date, " and ", reporting_date_month_year," and only notified in ", reporting_date_month_year, ". The majority of those notifications were ", tb_back_captured," TB cases (**See Appendix no.1**).")
...
tbl_censor <- tbl_df2 %>% 
  select(censor, condition) %>% 
  tbl_summary(by = censor, 
              missing = "no",
              percent = "row",
              label = list(
                           condition ~ ""))%>%
...
merged_cat1<- tbl_merge(tbls = 
            list(tbl_province_cat2, tbl_censor), 
...
# Incidence of notified NMC conditions {.unnumbered}
...
source("SA_population_model.R") # calls data and does some cleaning
source("incidence_function.R")
source("gg2word.R")
...
gg2word(prov1$plot_case)
```

```{r, fig.cap = ""}
gg2word(prov1$plot_incidence)
```
...
gg2word(prov2$plot_case)
```

```{r, fig.cap = ""}
gg2word(prov2$plot_incidence)
```
\newpage 
...
gg2word(prov3$plot_cases)
```

```{r}
gg2word(prov3$plot_incidence)
```
\newpage 
...
gg2word(age1$plot_case)
```

```{r}
gg2word(age1$plot_incidence)
```
...
gg2word(age2$plot_case)
```

```{r}
gg2word(age2$plot_incidence)
```
...
gg2word(age3$plot_case)
```

```{r}
gg2word(age3$plot_incidence)
```
...
gg2word(TB_prov$plot_case)
```

```{r, warning = FALSE, message = FALSE}
# Cases Age and province
gg2word(TB_prov$plot_incidence)
```
...
gg2word(TB_age$plot_case)
```


```{r, warning = FALSE, message = FALSE}
# Cases Age and province
gg2word(TB_age$plot_incidence)
```
...
gg2word(hep_prov$plot_case)
```

```{r, warning = FALSE, message = FALSE}
# Cases Age and province
gg2word(hep_prov$plot_incidence)
```
...
gg2word(hep_age$plot_case)
```

```{r, warning = FALSE, message = FALSE}
# Cases Age and province
gg2word(hep_age$plot_incidence)
```
